...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self):
        """
        Return True iff the Sudoku Puzzle cannot be solved
        ...
        if SudokuPuzzle.is_solved(self):
            return False
        # else check to see if it can't be
        # Check:if open position has symbol within same row, column or subsquare
        else:
            for symbol in self._symbols:
                if symbol == "*":
                    for i in range(len(self._symbols)):
                        rows = self._row_set(i)
                        cols = self._column_set(i)
                        subsquare = self._subsquare_set(i)
                        uni = rows | cols | subsquare
                        if (len(self._symbol_set - uni)) <= 0:
                            return True
        return False
