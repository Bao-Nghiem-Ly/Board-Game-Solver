...
def depth_first_solve(puzzle):
    ...
    if puzzle.fail_fast():
        return None
    path = _do_depth(puzzle, set(str(puzzle)))
    return path


def _do_depth(puzzle, seen):
    """
    Return a path from PuzzleNode(puzzle) to a PuzzleNode containing
    a solution, with each child containing an extension of the puzzle
    in its parent.  Return None if this is not possible.
    ...
    if puzzle.is_solved():
        ...
        for extension in puzzle.extensions():
            if str(extension) not in seen:
                temp = _do_depth(extension, seen)
                if isinstance(temp, PuzzleNode):
                    return PuzzleNode(puzzle, [temp])
        return None


def construct_path(node):
    """
    Return the path from node to it's root
...
def breadth_first_solve(puzzle):
    ...
    while queue:
        curr_node = queue.pop()
        curr_puzzle = curr_node.puzzle
        if curr_puzzle.fail_fast():
            return None
        elif curr_puzzle.is_solved():
            return construct_path(curr_node)
        for extension in curr_puzzle.extensions():
            if str(extension) not in seen_puzzles:
                next_node = PuzzleNode(extension)
                next_node.parent = curr_node
                queue.appendleft(next_node)
                seen_puzzles.add(str(next_node.puzzle))
